...
export class ValidateForm {
  constructor(submitCallback, confOpt = {}) {
    ...
    this.doc = this.scope?.ownerDocument || document;
    this.accessibility = confOpt.accessibility !== false;

    this.numWarnings = 0;
    this.texts = {
      requiredField: confOpt.texts?.requiredField || 'campo requerido',
      wrongValue: confOpt.texts?.wrongValue || 'valor incorrecto',
    };
    ...
    const defaultValidators = {
      ...
      nif: VerifyUtils.validaNifCifNie,
      cif: VerifyUtils.validaNifCifNie,
      nie: VerifyUtils.validaNifCifNie,
    ...
    this.handlers = {
      ...
      onSelect: this._onSel.bind(this),
      showHidden: this._showHidden.bind(this),
    };
  ...
  _checkSubmitElements(formToValidate) {
    ...
    submitElementsToCheck.forEach((submitElementToCheck) => {
      ...
      submitBtn.addEventListener('click', this.handlers.beforeSubmit, false);
    });
  }
  ...
  validate(val, type, el) {
    if (val === '' || val === null || (val !== '' && type === 'noempty')) { return true; }
    if (type === 'selected' || type === 'noempty') { return (val !== ''); }
    if (type.substr(0, 3) === 'fn:') {
      const fn = globalThis[type.substr(3)];
      if (typeof fn === 'function') {
        return fn(val, el, this);
      }
    }
    if (type.substr(0, 5) === 'file:') {
      const validExtensions = type.substr(5).split(',');
      return this.validationTypeFunctions.file(val, validExtensions, el);
    }
    return (this.validationTypeFunctions[type]) ? this.validationTypeFunctions[type](val, el, this) : false;
  }

  addValidator(name, fn) {
    if (name && typeof fn === 'function') {
      this.validationTypeFunctions[name] = fn;
    }
  }

  removeValidator(name) {
    if (name && this.validationTypeFunctions[name]) {
      delete this.validationTypeFunctions[name];
    }
  }
  ...
  _drawAsterisk(el) {
    ...
    if (this.scope.querySelector(`#asterisco_${idAst}`) === null) {
      const asterisco = this.doc.createElement('span');
  ...
  addWarnMesg(_el, msg) {
    let el = _el;
    let warning;
    const target = (typeof el?.getAttribute !== 'undefined') ? el : null;
    if (!target) {
      return;
    }
    ...
    if (!this.scope.querySelector(`#${id}`)) {
      const targetId = target.getAttribute('id') || el.id;
      if (targetId) {
        el = this.scope.querySelector(`#${targetId}`) || el;
      }
      warning = this.doc.createElement('p');
  ...
  delWarnMesg(el) {
    ...
    if (this.scope.querySelector(`#${warningId}`)) {
      if (target.parentElement.querySelector(`#${warningId}`)) {
        target.parentElement.removeChild(this.scope.querySelector(`#${warningId}`));
      } else {
        this.scope.querySelector(`#${warningId}`).parentElement.removeChild(this.scope.querySelector(`#${warningId}`));
      }
    }
    this._clearFieldInvalid(target, warningId);
    this.removeStyle(el);
  ...
  addEventsToCheckFieldsWhenBlur(form) {
    ...
    fields.forEach((field) => {
      ...
      if (validateIsRequired === 'true' || validateType !== '') {
        el.removeEventListener('blur', this.handlers.onBlur, false);
        el.addEventListener('blur', this.handlers.onBlur, false);
        if (typeF === 'checkbox' || typeF === 'radio') {
          el.removeEventListener('click', this.handlers.onBlur, false);
          el.addEventListener('click', this.handlers.onBlur, false);
        }
      }
    });
    selectF.forEach((select) => {
      ...
      if (validateIsRequired === 'true') {
        el.addEventListener('click', this.handlers.onSelect, false);
        el.addEventListener('change', this.handlers.onSelect, false);
        el.addEventListener('blur', this.handlers.onSelect, false);
      }
    });
  }
    ...
    toActivate.forEach((elToActivate) => {
      ...
      if (elAc) {
        ...
        elAcs.forEach((elAcEl) => {
          elAcEl.addEventListener('blur', this.handlers.showHidden, false);
          elAcEl.addEventListener('click', this.handlers.showHidden, false);
        });
      }
    });
  }
  ...
  _onBlur(e) {
    ...
    if (validField && validateType) {
      validField = this.validate(val, type, target);
      if (validField) {
        this.delWarnMesg(target);
      } else {
        this.badValue += 1;
  ...
  _setFieldInvalid(el, warningId) {
    if (!this.accessibility || !el?.setAttribute) {
      return;
    }
  ...
  _clearFieldInvalid(el, warningId) {
    if (!this.accessibility || !el?.setAttribute) {
      return;
    }
