export class VerifyUtils {
  ...
  static validaNifCifNie(a, elementOrOptions) {
    ...
    const cadenadni = 'TRWAGMYFPDXBNJZSQVHLCKE'; let posicion; let letra; let letradni; let
      suma;
    const options = elementOrOptions?.dataset
      ? { documentType: elementOrOptions.dataset.documentType }
      : (elementOrOptions || {});
    if (options.documentType === 'PASAPORTE') { return 1; }
    if (temp !== '') {
